...
void GameState::spawnEnemies() {
    for (const auto& pos : map.getEnemyPositions()) {
        enemies.emplace_back(make_unique<ChaserEnemy>(pos.x, pos.y, 50.0f, 150.0f, player));
    }
}
...
void GameState::changeMap(const string& newMapPath, bool useAlternativeSpawn) {
    ...
    if (!lobby) {
        map = Map(newMapPath, "assets/tilesets/tiles.png", "assets/tilesets/items.png", 32, { 6, 99, 5 }, { 62, 69, 70, 67, 65, 66, 68, 64 });
    }
    else {
